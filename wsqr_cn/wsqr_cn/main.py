...
class QReaderDecoder(Decoder):
    ...
    pip_name = "qreader"
    quark_link = "夸克网盘下载地址: [link=https://pan.quark.cn/s/cb0ad097711a]https://pan.quark.cn/s/cb0ad097711a[/]"
    ...
    def _test(cls):
        ...
        try:
            ...
            return False
        donwload_error_logged = False
        model_dir = pathlib.Path(qrdet.__file__).parent / ".model"
        model_dir.mkdir(exist_ok=True)
        src_model_file = model_dir / "qrdet-l.pt"
        local_model_dir = pathlib.Path(__file__).parent / "models"
        local_model_file = local_model_dir / "qrdet-l.pt"
        local_model_file_md5 = local_model_file.parent / (
            local_model_file.name + ".md5"
        )
        ...
        if os.path.isfile(local_model_file_md5):
            local_model_file_md5 = open(local_model_file_md5, "r").read().strip()
        if local_model_file.exists():
            use_local = True
            if (
                md5(open(local_model_file, "rb").read()).hexdigest()
                != local_model_file_md5
            ):
                use_local = not Confirm.ask(
                    "本地模型文件 {local_model_file} MD5校验失败, 是否重新下载",
                    default=False,
                )
            if use_local:
                log(f"[dim]复制本地模型文件: {local_model_file} 到 {src_model_file}[/]")
                shutil.copy(local_model_file, src_model_file)
                return True
        ...
        try:
            ...
            for dns in ["223.5.5.5", "8.8.8.8", "1.1.1.1"]:
                session = DNSClientSession(dns)

                try:
                    log(f"[dim]使用DNS服务器: {dns}[/]")
                    session = DNSClientSession(dns)
                            ...
                            with open(src_model_file, "wb") as f:
                                ...
                                for chunk in r.iter_content(chunk_size=1024):
                                    ...
                                    if (
                                        ...
                                        and downloaded < 0.3 * 1024 * 1024
                                    ):
                                        progress.stop()
                                        # 3s : 0.3MB = 0.3*1024*1024
                                        not_asked = False

                                        log(
                                            f"[link={url}]{url}模型文件下载速度过慢, 可手动下载 qrdet-l.pt 并放置到 {model_dir} 目录下[/]"
                                        )
                                        log(cls.quark_link)
                                        donwload_error_logged = True
                                        if not Confirm.ask(
                                            f"\n是否继续下载", default=True
                                        ):
        ...
        if not donwload_error_logged:
            log(
                f"\n\n[link={url}]{url}模型文件下载失败, 请手动下载 qrdet-l.pt 并放置到 {model_dir} 目录下[/]"
            )
            log(cls.quark_link)
